...
export const shopOwnerAuthentication = async (req, res, next) => {
  try {
    if (req.headers.authorization) {
      let token = req.headers.authorization.split(" ")[1];
      let data = await jwt.decode(token);
      if (Math.floor(+new Date() / 1000) < data.exp) {
        if (data.data.role === "owner") {
          ...
          res
            .status(404)
            .send({ message: "Shop owner one use this page", rd: false });
        }
      } else {
        res.status(401).send({ message: "Token Expired", rd: false });
      }
...
export const marketpersonAuthentication = async (req, res, next) => {
  try {
    if (req.headers.authorization) {
      ...
      let data = await jwt.decode(token);
      if (Math.floor(+new Date() / 1000) < data.exp) {
        if (data.data.role === "market") {
          next();
        } else {
          res
            .status(404)
            .send({ message: "market person one use this page", rd: false });
        }
      } else {
        res.status(401).send({ message: "Token Expired", rd: false });
      }
